...
from sms_man import get_sms_code, get_temp_phone_number
        ...
        while True:
            try:
                ...
                if status_code == "complete" and not any(
                    error_code in driver.page_source for error_code in error_codes
                ):
                    return result
...
def browserSetup(isMobile: bool, user_agent: str = PC_USER_AGENT, proxy: str = None) -> WebDriver:
    ...
    if ARGS.edge:
        browser = (
            ...
            if ARGS.no_webdriver_manager
            else webdriver.Edge(service=Service(EdgeChromiumDriverManager().install()), options=options)
        )
    else:
        browser = (
...
def login(browser: WebDriver, email: str, pwd: str, totpSecret: str, isMobile: bool = False):
    ...
    def handleLockedAccount():
        ...
        # ************************************************************
        # SMS MAN phone numbers don't currently work for this use case
        # ************************************************************

        # if not ARGS.sms_man:
        #     raise AccountLockedException

        # # Try to unlock account with SMS MAN
        # print("[SMS-MAN] Trying to unlock account...")
        # try:
        #     # Get token, country id and extension number from arguments
        #     token = ARGS.sms_man[0]
        #     country_id = ARGS.sms_man[1]
        #     extension_number = ARGS.sms_man[2]

        #     # Define selector constants
        #     NEXT_BUTTON_ID = "StartAction"
        #     PHONE_NUMBER_FIELD_XPATH = "/html/body/div[1]/div/div/div[2]/div/div[1]/div[2]/div/div[2]/div[2]/div/div/section/div/div[3]/div/div/div[1]/div/div/div/div/div[2]/div[2]/input"
        #     COUNTRY_EXTENSION_SELECT_XPATH = "/html/body/div[1]/div/div/div[2]/div/div[1]/div[2]/div/div[2]/div[2]/div/div/section/div/div[3]/div/div/div[1]/div/div/div/div/div[2]/div[1]/select"
        #     SEND_SMS_BUTTON_XPATH = "/html/body/div[1]/div/div/div[2]/div/div[1]/div[2]/div/div[2]/div[2]/div/div/section/div/div[3]/div/div/div[1]/div/div/div/div/div[3]/a[1]"
        #     SMS_CODE_FIELD_XPATH = "/html/body/div[1]/div/div/div[2]/div/div[1]/div[2]/div/div[2]/div[2]/div/div/section/div/div[3]/div/div/div[1]/div/div/div/div/div[4]/div/input"
        #     SUBMIT_BUTTON_ID = "ProofAction"

        #     # Press next button to get to phone number field
        #     next_button = browser.find_element(By.ID, NEXT_BUTTON_ID)
        #     next_button.click()

        #     # Wait for phone number field to appear
        #     waitUntilVisible(
        #         browser,
        #         By.XPATH,
        #         PHONE_NUMBER_FIELD_XPATH,
        #     )

        #     # Choose correct country extension given by user
        #     country_extension_select = Select(
        #         browser.find_element(
        #             By.XPATH,
        #             COUNTRY_EXTENSION_SELECT_XPATH,
        #         )
        #     )
        #     for option in country_extension_select.options:
        #         if extension_number in option.text:
        #             country_extension_select.select_by_visible_text(option.text)
        #             break

        #     # Get temporary phone number
        #     phone_number = get_temp_phone_number(token, country_id)
        #     # Remove extension number from phone number
        #     number = phone_number.number.replace(extension_number.replace("+", ""), "", 1)

        #     # Enter phone number in number field
        #     number_field = browser.find_element(
        #         By.XPATH,
        #         PHONE_NUMBER_FIELD_XPATH,
        #     )
        ...
        #     send_sms_button = browser.find_element(
        #         By.XPATH,
        #         SEND_SMS_BUTTON_XPATH,
        #     )
        #     send_sms_button.click()

        #     # Wait for SMS code field to appear
        #     waitUntilClickable(browser, By.XPATH, SEND_SMS_BUTTON_XPATH)

        #     # Get SMS code
        #     sms_code = get_sms_code(token, phone_number)
        #     # Enter SMS code in SMS code field
        #     sms_code_field = browser.find_element(By.XPATH, SEND_SMS_BUTTON_XPATH)
        #     sms_code_field.send_keys(sms_code)

        #     # Submit SMS code and unlock account
        #     submit_button = browser.find_element(By.ID, SUBMIT_BUTTON_ID)
        #     submit_button.click()

        #     print("[SMS-MAN] Successfully unlocked the account!")
        # except (
        #     requests.RequestException,
        #     NoSuchElementException,
        #     ElementClickInterceptedException,
        # ) as exc:
        #     print("[SMS-MAN] Failed to unlock account!")
        #     raise AccountLockedException
    ...
    if ARGS.session:
        if browser.title == "":
            waitToLoadBlankPage()
        if browser.title == "Microsoft account privacy notice" or isElementExists(
            browser, By.XPATH, '//*[@id="interruptContainer"]/div[3]/div[3]/img'
        ):
            acceptNewPrivacy()
        ...
        if browser.title == "Microsoft account | Home" or isElementExists(browser, By.ID, "navs_container"):
            ...
            return
        elif browser.title == "Your account has been temporarily suspended" or browser.current_url.startswith(
            ...
            return
        elif browser.title == "Help us secure your account" or browser.current_url.startswith(
    ...
    try:
        ...
        if (
            ...
            or isElementExists(browser, By.CLASS_NAME, "serviceAbusePageContainer  PageContainer")
...
def RewardsLogin(browser: WebDriver):
    ...
    try:
        ...
        if (
            browser.find_element(By.XPATH, '//*[@id="error"]/h1').get_attribute("innerHTML")
    ...
    try:
        if isElementExists(browser, By.CLASS_NAME, "rewards-slide"):
            try:
                ...
                time.sleep(calculateSleep(5))
                progress, total = (
                    browser.find_element(By.XPATH, "//div[@class='rewards-slide']//mee-rewards-counter-animation/span")
                ...
                if progress < total:
                    browser.find_element(
                        By.XPATH,
                        "//mee-rewards-welcome-tour//mee-rewards-slide[contains(@class, 'ng-scope') and not(contains(@class,'ng-hide'))]//mee-rewards-check-mark/../a",
                    ).click()
                    time.sleep(calculateSleep(5))
            except:
                pass
    ...
    def completeDailySetQuiz(_activity: dict):
        ...
        for _ in range(numberOfQuestions):
            if numberOfOptions == 8:
                ...
                for i in range(8):
                    if (
                        browser.find_element(By.ID, "rqAnswerOption" + str(i)).get_attribute("iscorrectoption").lower()
                        == "true"
                    ):
                ...
                for i in range(4):
                    if (
                        browser.find_element(By.ID, "rqAnswerOption" + str(i)).get_attribute("data-option")
                        == correctOption
                    ):
                        # Click on later on Bing wallpaper app popup
                        if isElementExists(browser, By.ID, "b_notificationContainer_bop"):
...
def completePunchCards(browser: WebDriver):
        ...
        for child in childPromotions:
            if not child["complete"]:
                ...
                if child["promotionType"] == "quiz" and child["pointProgressMax"] >= 50:
                    browser.find_element(
                        By.XPATH, '//*[@id="rewards-dashboard-punchcard-details"]/div[2]/div[2]/div[7]/div[3]/div[1]/a'
                    ).click()
                    time.sleep(1)
                    browser.switch_to.window(window_name=browser.window_handles[1])
                    time.sleep(calculateSleep(15))
    ...
    for punchCard in punchCards:
        try:
            if (
                punchCard["parentPromotion"] is not None
                and punchCard["childPromotions"] is not None
                and punchCard["parentPromotion"]["complete"] is False
    ...
    def completeMorePromotionQuiz(_activity: dict):
        ...
        for _ in range(Questions):
            if numberOfOptions == 8:
                ...
                for i in range(4):
                    if (
                        browser.find_element(By.ID, f"rqAnswerOption{str(i)}").get_attribute("data-option")
                        == correctOption
                    ):
                        waitUntilClickable(browser, By.ID, f"rqAnswerOption{str(i)}", 25)
                        browser.find_element(By.ID, f"rqAnswerOption{str(i)}").click()
                        time.sleep(5)
                        if not waitUntilQuestionRefresh(browser):
                            return
                        break
    ...
    def completePromotionalItems():
        ...
        try:
            ...
            if (
                ...
                and item["destinationUrl"] == BASE_URL
            ):
...
def argumentParser():
    ...
    parser.add_argument("--headless", help="Enable headless browser.", action="store_true", required=False)
    parser.add_argument(
        "--session", help="Creates session for each account and use it.", action="store_true", required=False
    )
    parser.add_argument("--error", help="Display errors when app fails.", action="store_true", required=False)
    parser.add_argument(
        "--fast",
    ...
    parser.add_argument(
        "--edge", help="Use Microsoft Edge webdriver instead of Chrome.", action="store_true", required=False
    )
    parser.add_argument(
        "--account-browser",
        nargs=1,
    ...
    parser.add_argument("--calculator", help="MS Rewards Calculator", action="store_true", required=False)
    parser.add_argument("--skip-unusual", help="Skip unusual activity detection.", action="store_true", required=False)
    parser.add_argument(
        "--skip-shopping",
    ...
    parser.add_argument(
        "--no-images", help="Prevent images from loading to increase performance.", action="store_true", required=False
    )
    parser.add_argument(
        "--shuffle", help="Randomize the order in which accounts are farmed.", action="store_true", required=False
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--dont-check-for-updates", help="Prevent script from updating.", action="store_true", required=False
    )
    parser.add_argument(
        "--repeat-shopping",
    ...
    parser.add_argument(
        "--print-to-webhook", help="Print every message to webhook.", action="store_true", required=False
    )
    parser.add_argument(
        "--recheck-proxy", help="Rechecks proxy in case you face proxy dead error", action="store_true", required=False
    )
    parser.add_argument(
        "--accounts-file",
    ...
    parser.add_argument(
        "--sms-man",
        metavar=("<TOKEN>", "<COUNTRY_ID>", "<EXTENSION_NUMBER>"),
        nargs=3,
        help="This argument takes a token, a country id (run get_sms_man_country_id.py to check that) and a conutry extension (+34 for example in Spain) to be able to automatically get SMS codes when required.",
        type=str,
        required=False,
    )
    parser.add_argument(
        "--alias",
        metavar="<BOT_NAME>",
        help="Specify the name of the bot when sending logs to Telegram or Discord.",
        type=str,
        required=False,
    )
    ...
    try:
        ...
        for user in ACCOUNTS:
            ...
            if not user["username"] in LOGS.keys():
                LOGS[user["username"]] = {"Last check": "", "Today's points": 0, "Points": 0}
        ...
        for account in LOGS.keys():
            if LOGS[account]["Last check"] == str(date.today()) and list(LOGS[account].keys()) == [
                "Last check",
                "Today's points",
                "Points",
            ]:
    ...
    for index, value in enumerate(LOGS.items(), 1):
        ...
        if value[1]["Last check"] == str(date.today()):
            ...
            if redeem_message:
                ...
                message += "\n"
    if ARGS.currency:
        message += f"💵 Total earned points: {total_earned} " f"({format_currency(total_earned, ARGS.currency)}) \n"
        message += f"💵 Total Overall points: {total_overall} " f"({format_currency(total_overall, ARGS.currency)})"
    else:
        message += (
            f"💵 Total earned points: {total_earned} "
        ...
        message += (
            f"💵 Total Overall points: {total_overall} "
...
def setRedeemGoal(browser: WebDriver, goal: str):
    ...
    try:
        ...
        if "/" not in goal_progress.text.lower() or goal not in goal_name.text.lower():
            ...
            if not goal_found:
                prRed("[GOAL SETTER] Specified goal not found! Search for any typos...")
            else:
...
def redeemGoal(browser: WebDriver):
    ...
    try:
        ...
        try:
            ...
            id = url[-1]
            try:
                browser.find_element(By.XPATH, value=f'//*[@id="redeem-pdp_{id}"]').click()
                time.sleep(random.uniform(5, 7))
            except (NoSuchElementException, ElementClickInterceptedException):
                browser.find_element(By.XPATH, value="/html/body/div[1]/div[2]/main/div[2]/div[2]/div[3]/a[2]").click()
            time.sleep(random.uniform(5, 7))

            # Handle add details page
            if browser.find_elements(By.ID, value="idFirstName"):
                first_name = names.get_first_name()
                last_name = names.get_last_name()

                # Fill in details
                browser.find_element(By.ID, value="idFirstName").send_keys(first_name)
                browser.find_element(By.ID, value="idLastName").send_keys(last_name)

                # Press next button
                browser.find_element(By.ID, value="iNext").click()
                time.sleep(random.uniform(5, 7))
        ...
        try:
            ...
            if veri.lower() == "phone verification":
                if not ARGS.sms_man:
                    ...
                    print("[REDEEM] Phone verification required! Attempting to automatically verify with SMS MAN...")
                    try:
                        # Get token, country id and extension number from arguments
                        token = ARGS.sms_man[0]
                        country_id = ARGS.sms_man[1]
                        ...
                        # Define selector constants
                        NUMBER_FIELD_ID = "redeem-checkout-challenge-fullnumber"
                        SEND_BUTTON_ID = "redeem-checkout-challenge-validate"
                        CODE_FIELD_ID = "redeem-checkout-challenge-code"
                        REDEEM_BUTTON_ID = "redeem-checkout-challenge-confirm"

                        # Get temporary phone number
                        phone_number = get_temp_phone_number(token, country_id)
                        # Remove + from extension number so we can remove the extension number from the phone number
                        number = phone_number.number.replace(extension_number.replace("+", ""), "", 1)

                        # Write phone number that will be used for verification
                        number_field = browser.find_element(By.ID, NUMBER_FIELD_ID)
                        number_field.send_keys(number)

                        # Continue to receive SMS part
                        send_button = browser.find_element(By.ID, SEND_BUTTON_ID)
                        send_button.click()

                        waitUntilVisible(browser, By.ID, CODE_FIELD_ID, 30)
                        code_field = browser.find_element(By.ID, CODE_FIELD_ID)

                        # Retrieve SMS code
                        sms_code = get_sms_code(token, phone_number)
                        # Write SMS code
                        code_field.send_keys(sms_code)
                        redeem_button = browser.find_element(By.ID, REDEEM_BUTTON_ID)
                        ...
                        prRed("[REDEEM] Failed to get SMS code!")
                        prRed(str(exc))
                        LOGS[CURRENT_ACCOUNT]["Auto redeem"] = "Phone verification required!"
                        updateLogs()
                        cleanLogs()
...
def loadAccounts():
    ...
    try:
        ...
        prPurple(
            f"[ACCOUNT] Accounts credential file '{ACCOUNTS_PATH.name}' created."
...
def update_handler(local_version):
    ...
    def loadingbar(configuration: dict, skip_text_after_loading_bar_finished) -> None:
        ...
        for i in range(configuration["size_of_loading_bar"]):
            sys.stdout.write(
                configuration["text_before_loading_bar"]
                + "[{0}]   \r".format(
                    configuration["design_of_loaded_bar"] * (i + 1)
                    + configuration["design_of_unloaded_bar"] * ((configuration["size_of_loading_bar"] - 1) - i)
                )
            )
            sys.stdout.flush()
...
def farmer():
    ...
    global ERROR, MOBILE, CURRENT_ACCOUNT, STARTING_POINTS, POINTS_COUNTER  # pylint: disable=global-statement
    try:
        for account in ACCOUNTS:
            CURRENT_ACCOUNT = account["username"]
            ...
            if not LOGS[CURRENT_ACCOUNT]["PC searches"]:
                ...
                if ARGS.redeem:
                    if "goal" in account and not account["goal"].lower() in redeem_goal_title:
                        ...
                        print("[REDEEM] Goal has not been defined for this account, defaulting to Amazon gift card...")
                        goal = "amazon"
                    else:
                ...
                if not ARGS.skip_shopping and not LOGS[CURRENT_ACCOUNT]["MSN shopping game"]:
                    ...
                    if ARGS.repeat_shopping and not finished:
                        prYellow("[MSN GAME] Running MSN shopping again...")
                        finished = completeMSNShoppingGame(browser)
            ...
            if MOBILE:
                browser = browserSetup(
                    True, account.get("mobile_user_agent", MOBILE_USER_AGENT), account.get("proxy", None)
                )
